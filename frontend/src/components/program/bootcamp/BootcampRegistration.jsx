...
import { API_BASE, apiFetch } from '../../../lib/api'; // adjust the path to your api.js
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const options = {
        ...
        amount: order.amount,
        currency: "INR",
        name: "DLithe Bootcamp",
        description: `Payment for ${formData.program_name || "Bootcamp"}`,
        order_id: order.id,
        prefill: {
          name: formData.full_name,
          email: formData.email,
        ...
        handler: async (razorpayResponse) => {
          try {
            ...
            if (verifyRes.data?.success) {
              ...
              setTimeout(() => {
                navigate("/program/bootcampcards", { replace: true });
              }, 6000);
            } else {
              toast.error(
      ...
      <Card className="bg-white rounded-2xl shadow-2xl p-8 flex flex-col gap-6 border border-gray-200 !text-black">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl text-red-700 leading-snug">
           Transforming bootcamp into lasting learning
          </h2>
        </div>
        ...
        <div>
          <div className="flex items-center gap-2 font-semibold !text-black text-lg mb-2">
            <Star size={18} className="text-yellow-500 fill-yellow-500" /> Bootcamp Highlights
          </div>
