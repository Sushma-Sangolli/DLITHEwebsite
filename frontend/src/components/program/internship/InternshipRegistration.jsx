...
const InternshipRegistration = () => {
  ...
  // Map UI names to internal state and backend names
  const handleChange = (e) => {
    const { name, value } = e.target;

    // Keep reg_no in sync with usn field
    if (name === 'usn') {
      setFormData(prev => ({ ...prev, usn: value, reg_no: value }));
      return;
    }

    // Map technology_domain input to branch (Category)
    if (name === 'technology_domain') {
      setFormData(prev => ({ ...prev, technology_domain: value, branch: value }));
      return;
    }

    // Map UI mode -> program_mode (backend expects program_mode)
    if (name === 'mode') {
      const programModeValue = value === 'Online' ? 'NA' : value; // Online -> NA, Offline -> keep location
  ...
  // optional: you still have a coupon-apply UI; backend will validate during order creation too
 ...
 const handleRegister = async () => {
  ...
  try {
    ...
    const { order, key, tempTxnId, finalPrice, couponInfo } =
      createOrderRes.data;
    ...
    // 2️⃣ Razorpay options (moved here so we can call later)
    const options = {
      key,
      amount: order.amount,
      currency: "INR",
      name: "DLithe Internship",
      description: `Payment for ${formData.program_name || "Internship"}`,
      order_id: order.id,
      prefill: {
        name: formData.full_name,
        email: formData.email,
      ...
      handler: async (razorpayResponse) => {
        try {
          ...
          if (verifyRes.data?.success) {
            ...
            setTimeout(() => {
              navigate("/program/internshipcards");
            }, 6000);
          } else {
            toast.error(
    ...
    // ✅ If coupon applied → show popup, then open Razorpay AFTER toast closes
    if (couponInfo) {
      ...
      // 🚀 No coupon → open Razorpay immediately
      const rzp = new window.Razorpay(options);
      rzp.open();
    }
  } catch (err) {
      ...
      <Card className="bg-white rounded-2xl shadow-2xl p-8 flex flex-col gap-6 border border-gray-200 !text-black">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl text-red-700 leading-snug">
           Transforming internships into lasting learning
          </h2>
        </div>
        ...
        <div>
          <div className="flex items-center gap-2 font-semibold !text-black text-lg mb-2">
            <Star size={18} className="text-yellow-500 fill-yellow-500" /> Internship Highlights
          </div>
...
<motion.div variants={formVariants} whileHover={{ scale: 1.01 }}>
  ...
  <Select onValueChange={(val) => setFormData({ ...formData, prefix: val })}>
    <SelectTrigger
      className="w-full  truncate bg-gray-600 !text-black border border-gray-300 rounded-lg shadow-sm 
...
<motion.div variants={formVariants} whileHover={{ scale: 1.01 }}>
  ...
  >
    <SelectTrigger
      className="w-full truncate break-words bg-gray-600 !text-black border border-gray-300 rounded-lg shadow-sm 
                 focus:ring-2 focus:ring-gray-400 focus:border-gray-400 
                 data-[state=open]:!bg-gray-200 data-[placeholder]:!text-black !bg-gray-100"
    >
    ...
    <SelectContent className="bg-white truncate !text-black max-h-60 overflow-y-auto">
      <SelectItem value="Artificial Intelligence & Data Science" className="break-words whitespace-normal">Artificial Intelligence & Data Science</SelectItem>
      <SelectItem value="Artificial Intelligence & Machine Learning Engineering" className="break-words whitespace-normal">Artificial Intelligence & Machine Learning Engineering</SelectItem>
      <SelectItem value="Biotechnology Engineering" className="break-words whitespace-normal">Biotechnology Engineering</SelectItem>
      <SelectItem value="Civil Engineering" className="break-words whitespace-normal">Civil Engineering</SelectItem>
      <SelectItem value="Computer & Communication Engineering"className="break-words whitespace-normal">Computer & Communication Engineering</SelectItem>
      <SelectItem value="Computer Science & Engineering" className="break-words whitespace-normal">Computer Science & Engineering</SelectItem>
      <SelectItem value="Computer Science & Engineering (Cyber Security)" className="break-words whitespace-normal">Computer Science & Engineering (Cyber Security)</SelectItem>
      <SelectItem value="Cyber Security">Cyber Security</SelectItem>
      <SelectItem value="Electrical & Electronics Engineering"className="break-words whitespace-normal">Electrical & Electronics Engineering</SelectItem>
      <SelectItem value="Electric Vehicle Technology" className="break-words whitespace-normal">Electric Vehicle Technology</SelectItem>
      <SelectItem value="Electronics & Communication Engineering" className="break-words whitespace-normal">Electronics & Communication Engineering</SelectItem>
      <SelectItem value="Electronics Engineering (VLSI Design & Technology)" className="break-words whitespace-normal">Electronics Engineering (VLSI Design & Technology)</SelectItem>
      <SelectItem value="Electronics & Communication (Advanced Communication Technology)" className="break-words whitespace-normal">Electronics & Communication (Advanced Communication Technology)</SelectItem>
